...
const randomBytesAsync = util.promisify(require('crypto').randomBytes); // NOTE: Code from https://github.com/then/nodeify/blob/master/index.js

let nextTick = function (cb) {
  setTimeout(cb, 0);
};

if (typeof setImmediate === 'function') nextTick = setImmediate;else if (typeof process === 'object' && process && process.nextTick) nextTick = process.nextTick;
...
function enforceNotNull(func, message) {
  if (!func) throw new RandomGenerationError('Invalid func.');
  if (func()) throw new RandomGenerationError(message);
}
...
module.exports = function secureRandomNumber(minimum, maximum, cb) {
  let promise = Promise.resolve().then(() => {
    ...
    enforceNotNull(() => {
      return !minimum;
    }, 'You must specify a minimum value.');
    enforceNotNull(() => {
      return !maximum;
    }, 'You must specify a maximum value.');
    enforceNotNull(() => {
      return minimum % 1 !== 0;
    ...
    enforceNotNull(() => {
      return minimum < -9007199254740991 || minimum > 9007199254740991;
    }, 'The minimum value must be inbetween MIN_SAFE_INTEGER and MAX_SAFE_INTEGER.');
    enforceNotNull(() => {
      return maximum < -9007199254740991 || maximum > 9007199254740991;
    }, 'The maximum value must be inbetween MIN_SAFE_INTEGER and MAX_SAFE_INTEGER.');
    let range = maximum - minimum;
    enforceNotNull(() => {
      return range < -9007199254740991 || range > 9007199254740991;
    ...
    return Promise.resolve().then(() => {
      ...
      for (let i = 0; i < bytesNeeded; i++) randomValue |= randomBytes[i] << 8 * i;
