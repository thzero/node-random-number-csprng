...
function enforceNotNull(func, message) {
	if (!func)
		throw new RandomGenerationError('Invalid func.');
	if (func())
		throw new RandomGenerationError(message);
}

module.exports = function secureRandomNumber(minimum, maximum, cb) {
	let promise = Promise.resolve().then(() => {
		enforceNotNull(() => { return !randomBytesAsync; }, 'No suitable random number generator available. Ensure that your runtime is linked against OpenSSL (or an equivalent) correctly.');
		enforceNotNull(() => { return !minimum; }, 'You must specify a minimum value.');
		enforceNotNull(() => { return !maximum; }, 'You must specify a maximum value.');
		enforceNotNull(() => { return (minimum % 1 !== 0); }, 'The minimum value must be an integer.');
		enforceNotNull(() => { return (maximum % 1 !== 0); }, 'The maximum value must be an integer.');
		enforceNotNull(() => { return (!(maximum > minimum)); }, 'The maximum value must be greater than the minimum value.');
		...
		enforceNotNull(() => { return (minimum < -9007199254740991 || minimum > 9007199254740991); }, 'The minimum value must be inbetween MIN_SAFE_INTEGER and MAX_SAFE_INTEGER.');
		enforceNotNull(() => { return (maximum < -9007199254740991 || maximum > 9007199254740991); }, 'The maximum value must be inbetween MIN_SAFE_INTEGER and MAX_SAFE_INTEGER.');

		let range = maximum - minimum;
		enforceNotNull(() => { return (range < -9007199254740991 || range > 9007199254740991); }, 'The range between the minimum and maximum value must be inbetween MIN_SAFE_INTEGER and MAX_SAFE_INTEGER.');
